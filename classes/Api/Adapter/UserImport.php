...
class UserImport implements UserImportInterface
{
    ...
    protected function transformParticipantsToXml(
        ...
        foreach ($participants as $participant_container) {
            ...
            // Role assignment
            $participant_role = ($import_mode === UserImportInterface::IMPORT_MODE_SIFA)
    ...
    protected function fetchUserId(
        ...
        if (!ilObjUser::_exists($obj_id)) {
            ...
            throw $this->exception_factory->userImport(Message::NULL, 'Invalid db structure. Check log file. Aborting');
        }
    ...
    protected function writeOrganisationInfo(Organisation $org) : void
    {
        ...
        foreach ($this->settings->udfFieldsAsArray() as $field_name => $field_id) {
            $value = '';
            $field_update_required = false;
            switch ($field_name) {
                case SettingsName::UDF_SUPERVISOR->value:
                    $field_update_required = true;
                    $value = $org->getAufsichtspersonName();
                    break;

                case SettingsName::UDF_SUPERVISOR_EMAIL->value:
                    $field_update_required = true;
                    $value = $org->getAufsichtspersonEMail();
                    break;

                case SettingsName::UDF_MEMBER_ID->value:
                    $field_update_required = true;
