    ...
    protected function importTrainingCourseTrain(
        ...
        if (!$course instanceof ilObjCourse) {
            ...
            $this->builder_factory->mailSegment()
                ...
                ->store();
            throw $this->exception_factory->memberImport(Message::NULL, $message);
        }
        ...
        $veda_crs = $this->builder_factory->courseStatus()
            ->withOID($oid)
            ->get();
    ...
    protected function handleTutorAssignments(
        ...
        foreach ($participants->getTutors() as $tutor_id) {
            ...
            foreach ($remote_companions as $remote_companion) {
                if (!$this->utils->isValidDate($remote_companion->getZustaendigAb(), $remote_companion->getZustaendigBis())) {
            ...
            foreach ($remote_supervisors as $remote_supervisor) {
                if (!$this->utils->isValidDate($remote_supervisor->getKursZugriffAb(), $remote_supervisor->getKursZugriffBis())) {
                    $this->logger->debug('Ignoring supervisor outside time frame: ' . $remote_supervisor->getAufsichtspersonId());
                    continue;
                }
                if ($this->utils->compareOidsEqual($remote_supervisor->getAufsichtspersonId(), $supervisor_oid)) {
                    $found = true;
                    break;
                }
            }
        ...
        foreach ($remote_companions as $remote_companion) {
            ...
            $this->logger->dump($this->udf_db->getUsersForCompanionId($companion_id), ilLogLevel::DEBUG);

            if (!$this->utils->isValidDate($remote_companion->getZustaendigAb(), $remote_companion->getZustaendigBis())) {
                $this->logger->info('Outside time frame: Ignoring companion with id: ' . $companion_id);
                continue;
            }
            foreach ($this->udf_db->getUsersForCompanionId($companion_id) as $uid) {
                if (!in_array($uid, $participants->getTutors())) {
        ...
        foreach ($remote_supervisors as $remote_supervisor) {
            ...
            $this->logger->dump($this->udf_db->getUsersForSupervisorId($supervisor_id), ilLogLevel::DEBUG);

            if (!$this->utils->isValidDate($remote_supervisor->getKursZugriffAb(), $remote_supervisor->getKursZugriffBis())) {
                $this->logger->info('Outside time frame: Ignoring supervisor with id: ' . $supervisor_id);
                continue;
            }
            foreach ($this->udf_db->getUsersForSupervisorId($supervisor_id) as $uid) {
                if (!in_array($uid, $participants->getTutors())) {
