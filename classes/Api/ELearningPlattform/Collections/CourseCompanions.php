...
class CourseCompanions implements CourseCompanionsInterface
{
    ...
    public function containsCompanionWithOID(
        ...
        foreach ($this->crs_supervisors as $supervisor) {
            if (
                !$this->utils->compareOidsEqual($oid, $supervisor->getElearningbenutzeraccountId()) ||
                !$this->utils->isValidDate($supervisor->getKursZugriffAb(), $supervisor->getKursZugriffBis())
            ) {
                continue;
            }
            return true;
